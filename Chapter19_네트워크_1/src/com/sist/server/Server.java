 ...
 *     ------------------
 *         변수
 *          - ss:ServerSocket (- : private, + : public)
 *          - PORT: int
 *          - waitVc: Vector
 *     ------------------
 *         메소드
 ...
 *          +run : void : 쓰레도 통신 (클라이언트와 연동)
 *            => case 문장 이용
 *          +messageTo(String msg) : void : 반복제거 => 한명에게 데이터 전송 
 *          +messageAll(String msg) : void : 접속자 모두에게 전송
 *       -----------
 *        --------
 *        Server : 서버역할, 클라이언트 연결 관리, 메시지 전송
...
public class Server implements Runnable {
	...
	class Client extends Thread {
		...
		String id,name,address,pos;
		Socket s;
		BufferedReader in; // 접속자 요청 => HttpServletRequest
		OutputStream out; // 접속자 요청 처리에 대한 응답 => HttpServletResponse
